...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.status(400).send('Não existem dados!');
  }
});
...
router.delete('/delete/:id', async (req, res) => {
  try {
    ...
    let json = storage.grades.filter(
...
router.get('/search/:id', async (req, res) => {
  try {
    ...
    let search = storage.grades.filter(
      (student) => student.id === parseInt(req.params.id, 10)
    );
...
router.get('/:student/:subject', async (req, res) => {
  ...
  try {
    ...
    if (data.length != 0) {
      res.send(
        `O resultado para o aluno ${student} para a matéria ${subject} é: ${result}`
      );
      res.end();
    } else {
      throw new error();
...
router.get('/consulta/:subject/:type', async (req, res) => {
  ...
  try {
    async function calculation(event) {
      let i = 0;
      let average = event.grades
        .filter((searchType) => searchType.type == type)
    ...
    res.send(
      `A média das notas da matéria ${subject} e tipo ${type} é: ${result}`
    );
    res.end();
  } catch (error) {
